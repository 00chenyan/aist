...
public class CodeAnalyzeController {
    ...
    public ResponseEntity<Map<String, Object>> getProjectInfo() {
        Map<String, Object> result = new HashMap<>();
        result.put("success", true);
        result.put("data", codeAnalyzeService.getProjectInfo());
        return ResponseEntity.ok(result);
    }

    /**
     * 对 AI 回答进行点赞/点踩评价
     * 仅会落库到 conversation_record 表中 session_type = 2(ANSWER) 的记录上，
     * 且只会更新当前尚未评价的最新一条记录。
     */
    @PostMapping("/rate")
    public ResponseEntity<Map<String, Object>> rateAnswer(@RequestBody Map<String, Object> body) {
        Map<String, Object> result = new HashMap<>();
        ...
        Object thumbStatusObj = body.get("thumbStatus");

        String sessionId = sessionIdObj instanceof String ? (String) sessionIdObj : null;
        Integer thumbStatus = null;
        if (thumbStatusObj instanceof Number) {
            thumbStatus = ((Number) thumbStatusObj).intValue();
        }

        boolean updated = codeAnalyzeService.rateAnswer(sessionId, thumbStatus);
        if (updated) {
            result.put("success", true);
            result.put("message", "评价成功");
        } else {
            result.put("success", false);
            result.put("message", "评价失败或已评价过");
        }
        return ResponseEntity.ok(result);
    }
}
